...
public class ExtendedStatisticsImpl implements ExtendedStatistics {
    ...
    public ExtendedStatisticsImpl(StatisticsManager manager, ScheduledExecutorService executor, long timeToDisable, TimeUnit unit,
        ...
        this.cacheHitRatio = get().ratioOf(EnumSet.of(GetOutcome.HIT),
                EnumSet.allOf(GetOutcome.class));
        this.nonStopTimeoutRatio = nonstop().ratioOf(
                EnumSet.of(NonStopOperationOutcomes.REJOIN_TIMEOUT,
                        NonStopOperationOutcomes.TIMEOUT),
                EnumSet.allOf(NonStopOperationOutcomes.class));
    ...
    public Operation<SearchOutcome> search() {
        return (Operation<SearchOutcome>) getStandardOperation(StandardOperationStatistic.SEARCH);
    }
    ...
    public Operation<EvictionOutcome> eviction() {
        return (Operation<EvictionOutcome>) getStandardOperation(StandardOperationStatistic.EVICTION);
    }
    ...
    public Operation<ExpiredOutcome> expiry() {
        return (Operation<ExpiredOutcome>) getStandardOperation(StandardOperationStatistic.EXPIRY);
    }
