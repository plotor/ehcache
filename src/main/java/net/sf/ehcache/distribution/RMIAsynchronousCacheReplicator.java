 ...
 * <p>
 * Accordingly, the Element values in {@link EventMessage}s are held by {@link SoftReference} in the queue,
...
public class RMIAsynchronousCacheReplicator extends RMISynchronousCacheReplicator {
    ...
    private void replicationThreadMain() {
        while (true) {
            ...
            while (this.alive() && replicationQueue != null && replicationQueue.isEmpty()) {
                try {
    ...
    public final void notifyElementPut(final Ehcache cache, final Element element) throws CacheException {
        ...
        if (replicatePutsViaCopy) {
            ...
            this.addToReplicationQueue(new RmiEventMessage(cache, RmiEventType.PUT, null, element));
        } else {
            ...
            this.addToReplicationQueue(new RmiEventMessage(cache, RmiEventType.REMOVE, element.getKey(), null));
        }
    ...
    public void notifyRemoveAll(final Ehcache cache) {
        ...
        this.addToReplicationQueue(new RmiEventMessage(cache, RmiEventType.REMOVE_ALL, null, null));
    }
    ...
    private void writeReplicationQueue() {
        List<EventMessage> eventMessages = this.extractEventMessages(maximumBatchSize);
