...
public interface Ehcache extends Cloneable {
     ...
     * ({@link net.sf.ehcache.event.CacheEventListener#notifyElementPut(Ehcache, Element) notifyElementPut(...)})
     ...
     * @throws NullPointerException if the element is null, or has a null key
     * @see CacheConfiguration#addElementValueComparator(net.sf.ehcache.config.ElementValueComparatorConfiguration)
     */
    ...
    /**
     * Removes given set of {@link Element} from the Cache. This also removes them from any
    ...
    /**
     * Removes an {@link Element} from the Cache and any stores it might be in. This also removes through
     ...
     * by the cache's {@link CacheManager} instance during the cache
     ...
     * Should the cache already be initialized, {@link CacheLoader#init CacheLoader.init()} will not be invoked.
     ...
     *
     * @param coherent true transitions to coherent mode, false to incoherent mode
     ...
     * @throws CacheException if no such attribute is defined for the given name
     */
     ...
     * {@link NonstopConfiguration#getBulkOpsTimeoutMultiplyFactor()} times the timeout value in the nonstop config.
     *
     * @param enabledBulkLoad true enables bulk-load, false disables it if not already disabled
     * @throws UnsupportedOperationException if the cache is not clustered with Terracotta
     */
