...
/**
 * Abstract base class for criteria involving {@link Comparable} values
 *
 * @author teck
 */
public abstract class ComparableValue extends BaseCriteria {
    ...
    public boolean execute(Element e, Map<String, AttributeExtractor> attributeExtractors) {
        Object attrValue = getExtractor(this.getAttributeName(), attributeExtractors).attributeFor(e, this.getAttributeName());
        if (attrValue == null) {
            return false;
        } else {
            AttributeType attrType = AttributeType.typeFor(this.getAttributeName(), attrValue);
            if (!this.getType().equals(attrType)) {
                throw new SearchException("Expecting attribute of type " + this.getType().name() + " but was " + attrType.name());
            }

            if (this.getType().equals(AttributeType.STRING)) {
