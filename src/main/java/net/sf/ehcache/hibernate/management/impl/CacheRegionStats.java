...
public class CacheRegionStats implements Serializable {
    ...
    protected double determineHitRatio() {
        ...
        if (readCount > 0) {
            result = this.getHitCount() / ((double) readCount);
        }
        return result;
    }
    ...
    public String toString() {
        return "region=" + this.getRegion() + "shortName=" + this.getShortName() + ", hitCount=" + this.getHitCount() + ", missCount="
                + this.getMissCount() + ", putCount" + this.getPutCount() + ", hitRatio" + this.getHitRatio() + ", elementCountInMemory="
                + this.getElementCountInMemory() + ", elementCountOnDisk=" + this.getElementCountOnDisk() + ", elementCountTotal="
                + this.getElementCountTotal();
    }
    ...
    public CompositeData toCompositeData() {
        try {
            return new CompositeDataSupport(COMPOSITE_TYPE, ITEM_NAMES, new Object[] {this.getRegion(), this.getShortName(),
                    this.getHitCount(), this.getMissCount(), this.getPutCount(), this.getHitRatio(), this.getElementCountInMemory(),
                    this.getElementCountOnDisk(), this.getElementCountTotal(),});
        } catch (OpenDataException e) {
            throw new RuntimeException(e);
        }
    }
