...
public class MemoryStore extends AbstractStore implements CacheConfigurationListener, Store {
    ...
    protected MemoryStore(Ehcache cache, Pool pool, BackingFactory factory, final SearchManager searchManager) {
        ...
        int maximumCapacity = this.isClockEviction() && !storePinned ? maximumSize : 0;
    ...
    public void expireElements() {
        for (Object key : this.keySet()) {
            final Element element = this.expireElement(key);
            if (element != null) {
    ...
    public final boolean canPutWithoutEvicting(Element element) {
        ...
        return !this.isFull() && poolAccessor.canAddWithoutEvicting(element.getObjectKey(), element.getObjectValue(), map.storedObject(element));
    }
    ...
    private void checkCapacity(final Element elementJustAdded) {
        if (maximumSize > 0 && !this.isClockEviction()) {
    ...
    public void memoryCapacityChanged(int oldCapacity, int newCapacity) {
        ...
        if (this.isClockEviction() && !storePinned) {
