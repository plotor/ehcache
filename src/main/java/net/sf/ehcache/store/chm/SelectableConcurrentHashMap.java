...
public class SelectableConcurrentHashMap {
    ...
    private Set<Entry<Object, Element>> entrySet;
    ...
    public class Segment extends ReentrantReadWriteLock {
        ...
        public void recalculateSize(Object key, int hash) {
            ...
            if (value != null) {
                long delta = poolAccessor.replace(oldSize, key, value, SelectableConcurrentHashMap.this.storedObject(value), true);
                final WriteLock writeLock = this.writeLock();
                writeLock.lock();
                try {
                ...
                if ((onlyIfAbsent && oldValue != null || !onlyIfAbsent)) {
                    if (SelectableConcurrentHashMap.this.maxSize > 0) {
                        ...
                        while (runs-- > 0) {
                            ...
                            if (evict != null) {
                                Element removed;
                                while ((removed = this.remove(evict.getKey(), hash(evict.getKey().hashCode()), null)) == null) {
                                    evict = this.nextExpiredOrToEvict(value);
                                    if (evict == null) {
                                        break;
                                    }
                                }
        ...
        private Element nextExpiredOrToEvict(final Element justAdded) {
            ...
            while (i++ < count) {
                if (evictionIterator == null || !evictionIterator.hasNext()) {
                    evictionIterator = this.iterator();
                }
        ...
        boolean evict() {
            ...
            try {
                ...
                if (evict != null) {
                    if (cacheEventNotificationService != null) {
                        evictionObserver.begin();
                        remove = this.remove(evict.getKey(), hash(evict.getKey().hashCode()), null);
                        evictionObserver.end(EvictionOutcome.SUCCESS);
                    } else {
                        remove = this.remove(evict.getKey(), hash(evict.getKey().hashCode()), null);
                    }
    ...
    abstract class HashEntryIterator extends HashIterator {
        ...
        public HashEntryIterator() {
            myNextEntry = this.advanceToNextEntry();
        }
