...
public class Segment extends ReentrantReadWriteLock {
            ...
            if (count != 0) {
                ...
                while (e != null) {
                    if (e.hash == hash && key.equals(e.key)) {
                        ...
                        return this.decodeHit(e.element);
                    }
                    e = e.next;
                }
            }
    ...
    boolean fault(Object key, int hash, Placeholder expect, DiskMarker fault, final boolean skipFaulted) {
        this.writeLock().lock();
        try {
            return this.faultInternal(key, hash, expect, fault, skipFaulted);
        } finally {
            this.writeLock().unlock();
        }
    }
    ...
    private boolean faultInternal(final Object key, final int hash, final Placeholder expect, final DiskMarker fault, final boolean skipFaulted) {
        ...
        if (count != 0 && !faulted) {
            ...
            if (incomingDiskSize < 0) {
                ...
                final Element element = this.get(key, hash, false);
    ...
    /**
     * Remove the matching mapping.  Unlike the {@link Segment#remove(Object, int, net.sf.ehcache.Element, net.sf.ehcache.store.ElementValueComparator)} method
            ...
            while (e != null) {
                if (e.hash == hash && key.equals(e.key)) {
                    ...
                    if (diskSubstitute instanceof Placeholder) {
                        if (((Placeholder) diskSubstitute).hasFailedToFlush() && this.evict(key, hash, diskSubstitute) != null) {
                            diskSubstitute = null;
                        }
                    } else {
